...
# 設定網頁標題與排版
st.set_page_config(page_title="市場情緒量化儀表板 v3.1 (修正顯示版)", page_icon="📈", layout="wide")

st.title("📈 市場情緒量化儀表板 v3.1 (趨勢濾網版)")
...
s4 = get_vix_score(in_vix, is_uptrend)
s5 = get_bias_score(in_bias, is_uptrend)
...
with st.expander("📖 查看目前適用的給分標準 (點擊展開)", expanded=False):
    ...
    # 定義顯示函數 (使用 '至' 取代 '~' 避免 markdown 解析錯誤)
    def show_rules(uptrend_mode):
        ...
        # 修正: 使用 '至' 或空格隔開，避免 '~' 造成刪除線
        st.markdown("""
        * **恐懼貪婪**: ≤25 (+2), 26-44 (+1), 45-55 (0), 56-74 (-1), ≥75 (-2)
        * **McClellan**: ≤ -100 (+2), -100 至 -50 (+1), -50 至 +50 (0), +50 至 +100 (-1), > +100 (-2)
